...
function generateContributions(yearStart, memberId) {
    ...
    fetch(`fetch_mountzion_contributions.php?memberId=${memberId}`)
...
// The rest of your code remains the same.
    ...
    function populateMemberTable() {
        ...
        xhr.open("GET", "fetch_mountzion_member.php", true);
        xhr.onload = function() {
            if (xhr.status === 200) {
    ...
    function viewMembershipCard(memberId) {
        const xhr = new XMLHttpRequest();
        xhr.open("GET", `fetch_mountzion_member.php?id=${memberId}`, true);
        xhr.onload = function() {
            if (xhr.status === 200) {
    ...
    function addNewMember() {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "add_mountzion_member.php", true);
    ...
    document.getElementById('contributions').addEventListener('change', function(e) {
        if (e.target.type === 'checkbox') {
            ...
            const xhr = new XMLHttpRequest();
            xhr.open("POST", "update_mountzion_contribution.php", true);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xhr.onload = function() {
                if (xhr.status === 200) {
